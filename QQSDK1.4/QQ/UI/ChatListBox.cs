    ...
    public partial class ChatListBox : Control
    {
        ...
        public ChatListSubItem SelectSubItem
        {
            ...
            set
            {
                ...
                {
                    selectSubItem = value;
                    ChatListItem item = selectSubItem.OwnerListItem;
                    if (item != null)
                    {
                        //滑动 项
                        if (!item.IsOpen)//该项并没有展开.
                        {
                            ...
                            //获取 项的索引,用于计算位置.
                            int index = item.SubItems.IndexOf(selectSubItem);
                            pos += (index) * (int)ChatListItemIcon.Small + (int)ChatListItemIcon.Large;
                            //滑动到该位置.
                            chatVScroll.SetScroll(pos);
                        ...
                        {
                            //组 已经展开,直接获取项的位置.
                            chatVScroll.Value = selectSubItem.Bounds.Y;
                        }
                        ...
                        if (item.IsOpen)
                        {
                            foreach (ChatListSubItem subItem in item.SubItems)
                            {
                                if (subItem.Bounds.Contains(m_ptMousePos))
                                {
                                    ...
                                    {
                                        OnContextSubItem(new ChatListEventArgs(m_mouseOnSubItem, subItem));
        ...
        protected virtual void DrawHeadImage(Graphics g, ChatListSubItem subItem, Rectangle rectSubItem)
        {
            try
            {
                ...
                if (subItem.HeadImage == null)                 //如果头像位空 用默认资源给定的头像
                    subItem.HeadImage = global::CWebQQ.Properties.Resources._null;
                ...
                else
                {
                    ...
                    if (subItem.Status == ChatListSubItem.UserStatus.QMe)
                        g.DrawImage(global::CWebQQ.Properties.Resources.QMe, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                    if (subItem.Status == ChatListSubItem.UserStatus.Away)
                        g.DrawImage(global::CWebQQ.Properties.Resources.Away, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                    if (subItem.Status == ChatListSubItem.UserStatus.Busy)
                        g.DrawImage(global::CWebQQ.Properties.Resources.Busy, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                    if (subItem.Status == ChatListSubItem.UserStatus.DontDisturb)
                        g.DrawImage(global::CWebQQ.Properties.Resources.Dont_Disturb, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                }
        ...
        protected virtual void DrawSmallSubItem(Graphics g, ChatListSubItem subItem, Rectangle rectSubItem)
        {
            try
            {
                ...
                string strDraw =string.Format("{0}({1})", subItem.DisplayName, subItem.NicName);
        ...
        #region 公有函数
        public ChatListSubItem[] GetSubItemsById(Predicate<int> match)
        {
            ...
            for (int i = 0, lenI = this.items.Count; i < lenI; i++)
            {
                for (int j = 0, lenJ = items[i].SubItems.Count; j < lenJ; j++)
                {
                    if (match(items[i].SubItems[j].ID))
        ...
        /// <returns>列表子项的数组</returns>
        public ChatListSubItem[] GetSubItemsByNickName(string nicName)
        {
            List<ChatListSubItem> subItems = new List<ChatListSubItem>();
            for (int i = 0, lenI = this.items.Count; i < lenI; i++)
            {
        ...
        public ChatListSubItem[] GetSubItemsByDisplayName(Predicate<string> match)
        {
            List<ChatListSubItem> list = new List<ChatListSubItem>();
