...
namespace QQSDK.Net
{
    ...
    static class PasswordEncrypt
    {
        ...
        /// <returns></returns>
        public static string MD5Encrypt(string md5_str)
        {
        ...
        /// <returns></returns>
        private static string MD5Encrypt(byte[] md5_bytes)
        {
            System.Security.Cryptography.MD5 md5 = System.Security.Cryptography.MD5CryptoServiceProvider.Create();
        ...
        /// <returns></returns>
        private static byte[] MD5GetBytes(string md5_str)
        {
            System.Security.Cryptography.MD5 md5 = System.Security.Cryptography.MD5CryptoServiceProvider.Create();
            byte[] bytes = System.Text.Encoding.ASCII.GetBytes(md5_str);
        ...
        /// <returns></returns>
        public static string MD5QQEncrypt(long uin, string password, string verifyCode)
        {

            ByteBuffer buffer = new ByteBuffer();
            buffer.Put(MD5GetBytes(password));
            //buffer.Put(Encoding.UTF8.GetBytes(password));
            buffer.PutInt(0);
            buffer.PutInt((uint)uin);
            byte[] bytes = buffer.ToByteArray();
            string md5_1 = MD5Encrypt(bytes);//将混合后的字节流进行一次md5加密
            string result = MD5Encrypt(md5_1 + verifyCode.ToUpper());//再用加密后的结果与大写的验证码一起加密一次
            return result;
