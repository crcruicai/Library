...
namespace QQRobot.Util
{
    ...
    {
        #region 私有字段
        private string m_DicPath = Environment .CurrentDirectory +"/ShootSeg/sDict.dic";
        private string m_NoisePath =Environment .CurrentDirectory +"/ShootSeg/sNoise.dic";
        private string m_NumberPath = Environment .CurrentDirectory +"/ShootSeg/sNumber.dic";
        private string m_WordPath = Environment .CurrentDirectory +"/ShootSeg/sWord.dic";
        private string m_PrefixPath = Environment .CurrentDirectory +"/ShootSeg/sPrefix.dic";
    ...
    public class SegListTest
    {
        public static void Test()
        {
            ...
            string text =seg.SegmentText("字符串", false).Trim();
            Console.WriteLine(text);

            text = seg.SegmentText("下面是代码，有注释，很容易看懂的~有疑问请留言！", false).Trim();

            Console.WriteLine(text);
            text = seg.SegmentText("我也是刚开始学程序，代码可能不是很简洁的，希望大家改正~", false).Trim();
            Console.WriteLine(text);
            text = seg.SegmentText("最近考虑做个动感日记，其中一个需求就是像163邮箱里面，可以设置背景信纸。", false).Trim();
