...
namespace CRC.Controls
{
	...
	[DesignTimeVisible(false), TypeConverter("CRC.Controls.ListViewItemConverter")]
	...
	[DesignTimeVisible(false), TypeConverter("CRC.Controls.SubListViewItemConverter")]
	...
	[DesignTimeVisible(false), TypeConverter("CRC.Controls.ToggleColumnHeaderConverter")]
	...
	public class ColumnHeaderCollection: CollectionBase
	{
		...
		public virtual int Add(CRC.Controls.ToggleColumnHeader colhead)
		{
		...
		public virtual void AddRange(CRC.Controls.ToggleColumnHeader[] items)
		{
			lock(List.SyncRoot)
			{
				for (int i=0; i< items.Length; i++)
	...
	public class ContainerListView: Control
	{
		...
		protected CRC.Controls.ColumnHeaderCollection columns;			
		...
		protected CRC.Controls.ContainerListViewItem focusedItem;	
		...
		private void Construct()
		{
			...
			columns = new CRC.Controls.ColumnHeaderCollection();
		...
		public CRC.Controls.ColumnHeaderCollection Columns
		{
			get { return columns; }
		}
		...
		public bool	VisualStyles
		{
			get 
			{
				...
				try
				{
					val = visualStyles && CRC.Controls.uxTheme.Wrapper.IsAppThemed();
				}
				catch
				{
					val = visualStyles;
			...
			if (headerStyle != ColumnHeaderStyle.None)
			{
				...
				try
				{
					...
					for (i=0; i<columns.Count; i++)
					{
						colwid = (columns[i].ScaleStyle == ColumnScaleStyle.Spring ? springWid : columns[i].Width);
						if (headerStyle == ColumnHeaderStyle.Clickable && columns[i].Pressed)
							CRC.Controls.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "PRESSED", hdc,
								lp_scr+last, tp, 
								colwid, headerBuffer,
								lp, tp, r.Width-6, headerBuffer);
						else if (headerStyle != ColumnHeaderStyle.None && columns[i].Hovered)
							CRC.Controls.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "HOT", hdc,
								lp_scr+last, tp, 
								colwid, headerBuffer,  
								lp, tp, r.Width-6, headerBuffer);
						else
							CRC.Controls.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "NORMAL", hdc,
								lp_scr+last, tp, 
								colwid, headerBuffer,  
								lp, tp, r.Width-6, headerBuffer);
						last += colwid;
					...
					{
						CRC.Controls.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "NORMAL", hdc, lp_scr+last, tp, r.Width-last-2+hscrollBar.Value, headerBuffer,  r.Left, r.Top, r.Width, headerBuffer);
					}
				}
				catch
				{
